  ...
  override def onEvent(event: Event): Unit =
    event match {
      ...
      case MoneyTransferCreatedEvent(moneyTransferDetails, moneyTransferId)
           => accountService.creditAccount(moneyTransferDetails.accountIdFrom, moneyTransferDetails.amount, Option(moneyTransferId))

      case MoneyTransferStateCreditedEvent(moneyTransferDetails, moneyTransferId)
        => accountService.debitAccount(moneyTransferDetails.accountIdTo, moneyTransferDetails.amount, Option(moneyTransferId))
